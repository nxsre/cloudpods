...
func k8sPredicateFunc(args extenderv1.ExtenderArgs) *extenderv1.ExtenderFilterResult {
	pod := args.Pod
...
func k8sPrioritizeFunc(args extenderv1.ExtenderArgs) (*extenderv1.HostPriorityList, error) {
	nodes := args.Nodes.Items
	var priorityList extenderv1.HostPriorityList = make([]extenderv1.HostPriority, 0)
	for _, node := range nodes {
		priorityList = append(priorityList, extenderv1.HostPriority{
			Host:  node.Name,
			Score: 0,
		})
	}
